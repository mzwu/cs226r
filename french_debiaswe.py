...
from debiaswe.data import load_attributes_fr
from debiaswe.debias import debias

# load subtitle word embeddings
E = WordEmbedding('./data/french_subs.vec')

# load attributes
attributes = load_attributes_fr()
attributes_words = [p for p in attributes]

# Define gender direction
v_gender = E.diff("elle", "il")

# Load anology pairs
analogies_pair=set()

with open('./data/french_analogies.tsv', "r", encoding = "utf-8") as f:
    for line in f:
        if "#" in line:
            continue
        l = line.replace("\n", "").split("\t")
...
def evaluate_w2c(E, analogies_pair):
    ...
    for p in analogies_pair:
        ...
        if (male in word_vectors and fema in word_vectors):
            x = word_vectors.most_similar(positive=['homme', male], negative=['femme'])[0][0]
            print (male+"="+x+"-"+fema)
...
with open('./data/definitional_fr.json', "r", encoding = "utf-8") as f:
    defs = json.load(f)
print("definitional", defs)

with open('./data/equalize_fr.json', "r", encoding = "utf-8") as f:
    equalize_pairs = json.load(f)

with open('./data/gender_specific_fr.json', "r", encoding = "utf-8") as f:
